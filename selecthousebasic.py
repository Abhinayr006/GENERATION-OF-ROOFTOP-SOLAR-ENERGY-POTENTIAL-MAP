...
def redraw():
    ...
    for i, poly in enumerate(polygons):
        ...
        cv2.putText(display_img, f"H{i+1}", shifted[0],
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 2)

    for p in [(px - display_crop, py - display_crop) for px, py in points]:
...
print("👉 Select houses → N=save | Q=finish")
...
print("Polygons selected:", len(polygons))
...
if len(sys.argv) >= 3:
    ...
    print(f"Using Flask coords: {lat}, {lon}")
else:
...
try:
    ...
    print("⚠️ Using fallback solar values")
    irradiance_daily = 5.5
    avg_temp = 30
    avg_humidity = 60
    avg_wind = 2
